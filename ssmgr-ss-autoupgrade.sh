...
if [[ "$ssmgr_key" -eq "1" ]] && [[ "ssmgr_date" -eq "`date +%Y%m%d`" ]]; then
        rm -rf /root/shadowsocks-manager-tiny && cd /root && git clone https://github.com/siriusfinex/shadowsocks-manager-tiny.git
fi
...
if [[ "$ss_key" -eq "1" ]] && [[ "$ss_date" -eq "`date +%Y%m%d`" ]]; then
        cd /root && bash <(wget --no-check-certificate -qO- 'https://raw.githubusercontent.com/siriusfinex/linux-shell-bashscript/master/install-shadowsocks_libev-latest-version.sh')
fi
...
echo "35 3    * * *   root    bash /root/ssmgr-ss-autoupgrade.sh"
